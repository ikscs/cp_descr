...
import {
  ...
  Stack,
  OutlinedInput,
  Alert, // Добавил Alert для вывода сообщений об ошибках
} from '@mui/material';
...
interface UserFormProps {
  user?: User;
  onSave: (values: User) => Promise<void>; // Изменил тип onSave, чтобы он возвращал Promise
  onCancel: () => void;
  tenantId: string;
  title?: string;
}
...
const UserForm: React.FC<UserFormProps> = ({ user, onSave, onCancel, tenantId, title = 'Форма пользователя' }) => {
  ...
  const [submitError, setSubmitError] = useState<string | null>(null); // Состояние для хранения ошибки
  ...
  return (
    <Formik
      ...
      onSubmit={async (values, { setSubmitting, setStatus }) => {
        ...
        try {
          await onSave(values);
          setStatus({ success: 'Пользователь успешно сохранен!' });
          onCancel();
        } catch (error: any) {
          ...
          if (error.message)
            ...
            setSubmitError(error.response.data.message); // Устанавливаем сообщение об ошибке с бэкенда
          } else {
            setSubmitError('Не удалось сохранить пользователя. Пожалуйста, попробуйте еще раз.');
          }
        } finally {
          setSubmitting(false); // Отключаем состояние загрузки, независимо от результата
        }
      }}
    >
      {(formikProps: FormikProps<User>) => {
        const { values, isSubmitting, status } = formikProps;
        return (
          <Form>
