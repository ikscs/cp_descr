...
export const ReportToParsedReport = (report: Report): ParsedReport => {
  try {
    const configObject =
      report.config && typeof report.config === "string"
        ? JSON.parse(report.config)
        : null;
    const config =
      typeof configObject === "object" && configObject !== null
        ? configObject
    ...
    const chartConfig =
      typeof config.chart === "object" && config.chart?.type
        ? {
            ...
            body_fields: Array.isArray(config.chart.body_fields)
              ? config.chart.body_fields
    ...
    console.error(
      `Помилка розбору конфігурації для звіту ID ${report.id}:`,
      error,
      "Рядок конфігурації:",
      report.config
    );
...
const ReportList: React.FC<ReportListProps> = ({ reportFilterPredicate }) => {
  ...
  const handleExecuteReport = async (report: Report) => {
    ...
    if (parsedReport.config?.params?.length || 0 > 0) {
      setQueryParams(
        (parsedReport.config.params || []).map((p) => ({
          name: p.name,
  ...
  const executeReportQuery_ = async (
    ...
    try {
      ...
      if (
        ...
        return {
          columns: ["Повідомлення"],
          rows: [["Некоректний формат даних у відповіді"]],
        };
      }
      ...
      return {
        columns: ["Помилка"],
        rows: [[`Помилка запиту даних: ${err.message || "Невідома помилка"}`]],
      };
  ...
  const handleOpenChartDialog = (
    ...
    if (currentExecutionResult && selectedReport) {
      ...
      if (currentExecutionResult.rows.length === 0) {
        setError(
          "Немає даних для побудови графіка (результат виконання містить 0 рядків)."
        );
      ...
      if (
        ...
        setError(
          "Конфігурація графіка (тип, осі X, поля даних) не задана або неповна."
        );
      ...
      if (chartConfig.type === "circular" && yAxisIndices[0] === -1) {
        setError(
          `Поле для даних '${yAxisValueFields[0]}' не знайдено в результатах.`
        );
        setIsResultDialogOpen(true); // Fallback
        return;
      }
      ...
      if (
        ...
        setError(
          `Одне або декілька полей для даних (${missing.join(
            ", "
          )}) не знайдено в результатах.`
        );
        setIsResultDialogOpen(true); // Fallback
        return;
      }
      ...
      try {
        ...
        if (chartConfig.type === "circular") {
          ...
          if (yAxisIndices.length > 1) {
            console.warn(
              "Конфігурація кругової діаграми має декілька body_fields. Використовується перше:",
              yAxisValueFields[0]
            );
          }
        ...
        setError(
          `Помилка при обробці даних для графіка: ${processError.message}`
        );
        setIsResultDialogOpen(true); // Fallback
      }
    } else {
      setError(
        "Немає даних для побудови графіка (результат виконання відсутній або звіт не вибрано)"
      );
      setIsResultDialogOpen(true); // Fallback
    }
  };
      ...
      >
        ...
        <DialogContent>
          {selectedReport?.config?.params ? (
            ...
            <Typography>
              У цього звіту немає параметрів, що налаштовуються.
            </Typography>
          ) : (
            <CircularProgress />
          )}
        ...
        <DialogContent>
          {chartData && selectedReport && chartType ? ( // Убедимся, что все данные есть
            ...
            <Typography align="center" sx={{ mt: 4 }}>
              Немає даних для відображення графіка або тип графіка не визначено.
            </Typography>
          )}
        </DialogContent>
      </Dialog>
        ...
        <ReportResult
          ...
          handleOpenChartDialog={() => {
            ...
            if (executionResult) {
              ...
              setError(
                "Результати виконання звіту відсутні для побудови графіка."
              );
            }
          }}
