...
): { startDate: Date; endDate: Date; apiMode: ReportApiMode } => {
...
const PeriodMenu: React.FC<{
  ...
  <Menu anchorEl={anchorEl} open={open} onClose={onClose}>
    <MenuItem
      selected={activeGranularity === "DAY"}
      onClick={() => {
        onSelect("DAY");
        onClose();
      }}
    ...
    <MenuItem
      selected={activeGranularity === "WEEK"}
      onClick={() => {
        onSelect("WEEK");
        onClose();
      }}
    ...
    <MenuItem
      selected={activeGranularity === "MONTH"}
      onClick={() => {
        onSelect("MONTH");
        onClose();
      }}
    ...
    <MenuItem
      selected={activeGranularity === "YEAR"}
      onClick={() => {
        onSelect("YEAR");
        onClose();
      }}
    ...
    <MenuItem
      selected={activeGranularity === "PERIOD"}
      onClick={() => {
...
const DateRangeOrSinglePicker: React.FC<{
  ...
  if (!Array.isArray(value)) {
    console.error(
      "Inconsistent state in DateRangeOrSinglePicker: Received a single date value for a range granularity.",
      { granularity, value }
    );
...
const DashboardView: React.FC = () => {
  ...
  const [_parsedReport22, setParsedReport22] = useState<ParsedReport | null>(null);
  ...
  const [activeGranularity, setActiveGranularity] =
    useState<DashboardUserGranularity>("MONTH");
  ...
  const handleOpenDatePickerModal = useCallback(() => {
    ...
    const d1 = dayjs(
      reportParams.find((p) => p.name === "d1")?.value as string
    );
    if (activeGranularity === "DAY") {
      setTempDateValue(d1);
    } else {
      const d2 = dayjs(
        reportParams.find((p) => p.name === "d2")?.value as string
      );
  ...
  const handleApplyDateChange = () => {
    ...
    const d1 = isDay
      ? (tempDateValue as Dayjs)
      : (tempDateValue as [Dayjs, Dayjs])[0];
    const d2 = isDay
      ? (tempDateValue as Dayjs)
      : (tempDateValue as [Dayjs, Dayjs])[1];

    handleParamsUpdateFromControl([
      // d1.format() уже использует Dayjs, теперь с правильным форматом
  ...
  useEffect(() => {
    ...
    if (!isCustomerLoading) {
      // Only act if customer data loading is complete
      if (
        customerData &&
        customerData.points &&
        customerData.points.length > 0
      ) {
    ...
    setReportParams((prevParams) => {
      const currentPointValueInParams = prevParams.find(
        (p) => p.name === "point"
      )?.value;
      ...
      try {
        ...
        const handleResult = (
          ...
          if (result.status === "fulfilled") {
            ...
            const errorMsg = `Помилка завантаження звіту ${reportId}: ${
  ...
  const handleGranularityChange = useCallback(
    (newGranularity: DashboardUserGranularity) => {
      ...
      setReportParams((prevParams) => {
        const existingPointParam = prevParams.find(
          (p) => p.name === "point"
        ) || { name: "point", value: -1 };
        return [
          { name: "d1", value: dayjs(startDate).format(DATE_FORMAT_PARAM) }, // Форматируем с Dayjs
  ...
  const handleParamsUpdateFromControl = useCallback(
    (dateAndModeParams: ReportParameter[]) => {
      // Assuming dateAndModeParams contains {name: 'd1', ...}, {name: 'd2', ...}, {name: 'mode', ...}
      setReportParams((prevParams) => {
        const pointParam = prevParams.find((p) => p.name === "point") || {
          name: "point",
          value: -1,
        };
  ...
  const handlePointChange = useCallback(
    (event: SelectChangeEvent<string | number>) => {
      ...
      setReportParams((prevParams) =>
        prevParams.map((p) =>
          p.name === "point" ? { ...p, value: newPointId } : p
        )
      );
    },
    [setReportParams]
  ...
  const handleExport = async () => {
    ...
    const _pointLabel =
      customerData?.points?.find((p) => p.value === point_id)?.label ||
      "всі_пункти";
    const filename = `export_point_${_pointLabel}`;
    try {
      const data0 = await getExportData(
        point_id === -1 ? undefined : { point_id }
      );
  ...
  const currentPointSelected =
    (reportParams.find((p) => p.name === "point")?.value as number) ?? -1;
  ...
  const [periodMenuAnchor, setPeriodMenuAnchor] = useState<null | HTMLElement>(
    null
  );
  const openPeriodMenu = Boolean(periodMenuAnchor);
  const handleOpenPeriodMenu = (event: React.MouseEvent<HTMLElement>) =>
  ...
  const d1String = formatDateFns(
    new Date(reportParams.find((p) => p.name === "d1")?.value as string),
    "dd.MM.yyyy"
  );
  const d2String = formatDateFns(
    new Date(reportParams.find((p) => p.name === "d2")?.value as string),
    "dd.MM.yyyy"
  );
  const displayDate =
    activeGranularity === "DAY" ? d1String : `${d1String} - ${d2String}`;
    ...
    <LocalizationProvider dateAdapter={AdapterDayjs}>
      ...
      >
            ...
            >
              ...
              >
                ...
                <Tooltip 
                  ...
                  PopperProps={{
                    sx: {
                      zIndex: 1000, // Пример: понизить z-index тултипа
                    },
                  }}
                >
                  <span>
        ...
        <Dialog
          ...
          onClose={handleCloseDatePickerModal}
          maxWidth="md"
        >
          ...
          <DialogActions>
            ...
            <Button onClick={handleApplyDateChange} variant="contained">
              Застосувати
            </Button>
          </DialogActions>
        </Dialog>
